# Promevo Coding Assesment Submission

## Repo Contents

### Source Code

- ```./labels-ui``` contains the frontend app React project
- ```./labels-service``` contains the backend Spring project

### Demo/Testing Resources

- ```./labels-service-bruno``` contains a bruno collection to manually demo/test the labels-service app

## Quickstart

### Running the frontend app

1. if needed, install npm and vite
2. cd into ```./labels-ui```, then run ```npm install``` and ```npm run dev```
3. app will be available at http://localhost:5173

### Running the backend app

1. if needed, install maven and java 17
2. cd into ```./labels-service``` then run ```mvn clean install``` and ```mvn spring-boot:run```
3. app will be available at http://localhost:8080


## Dev Notes

### links/reference:

#### frontend

- [tanstack quickstart docs](https://tanstack.com/query/latest/docs/framework/react/quick-start)
- [react hook form quickstart docs](https://react-hook-form.com/get-started)
- [material ui install instructions](https://mui.com/material-ui/getting-started/installation/)
- [react router instructions](https://reactrouter.com/start/framework/routing)
- [react router - dynamic segments](https://reactrouter.com/start/data/routing#dynamic-segments)

#### backend 

- [gmail api quickstart](https://developers.google.com/workspace/gmail/api/guides)
- [gmail api auth docs](https://developers.google.com/workspace/guides/auth-overview)
- [google api auth help](https://support.google.com/googleapi/answer/6158857?hl=en&ref_topic=7013279&sjid=7103131004221423699-NC)
- [google api secrets json file format](https://developers.google.com/api-client-library/dotnet/guide/aaa_client_secrets)
- [gmail api client lib dependencies](https://developers.google.com/workspace/gmail/api/downloads)
- ['manage labels' gmail api docs](https://developers.google.com/workspace/gmail/api/guides/labels)